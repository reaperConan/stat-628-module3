...
color = ["red","sienna","gold","green","blue","purple","pink"]
for i in range(2,8):
...
sns.distplot(star)
len(friends[star==5])
sns.distplot(fans[fans<5])



#data1 = np.transpose(np.array([star,new]))
data = np.transpose([z_star,z_compliment,z_followers,z_review_count])
estimator = KMeans(n_clusters = 2)
estimator.fit(data)
pred = estimator.predict(data)
print(pred)
plt.scatter(friends[pred == 0],star[pred == 0],c = "red",s = 0.01,alpha = 0.2)
plt.scatter(friends[pred == 1],star[pred == 1],c = "blue",s=0.01,alpha = 0.2)
...
plt.scatter(friends[pred == 5],star[pred == 5],c = "yellow",s=0.01,alpha = 0.2)
plt.show()
#
#aa = pred[pred == 0]
